...
# Data
data <- xlsx::read.xlsx("Figures2_3_S1_Data.xlsx", sheetIndex = 1)
data$DaysFromDays = factor(data$DaysFromDays, levels = c("0", "15", "30", "60", "90", "180", "270", "360"))

# General plot

## IgM(N)
test = dunn_test(data, IgM~DaysFromDays)%>%filter(p.adj.signif != "ns")%>%add_xy_position(x = "DaysFromDays")
...
## IgG(N)
test = dunn_test(data, IgG~DaysFromDays)%>%filter(p.adj.signif != "ns")%>%add_xy_position(x = "DaysFromDays")
test$y.position = 15 +  c(1:nrow(test))
...
## IgG(S)
test = dunn_test(data%>%filter(DaysFromDays!=360), IgS~DaysFromDays)%>%filter(p.adj.signif != "ns")#%>%add_xy_position(x = "DaysFromDays")
